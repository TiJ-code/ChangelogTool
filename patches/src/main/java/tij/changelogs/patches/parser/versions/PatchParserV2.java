...
public final class PatchParserV2 {
    ...
    private static void parseComponents(Document doc, List<XmlComponent> components, ReducedConfig config) {
        ...
        for (int i = 0; i < componentNodes.getLength(); i++) {
            ...
            if (!element.hasAttribute(ATTRIBUTE_COMPONENT_REF)) {
                throw new RuntimeException(
                        "No attribute \"%s\" specified on <%s>"
                                .formatted(
                                        ATTRIBUTE_COMPONENT_REF,
                                        TAG_COMPONENT
                                )
                );
            }

            String ref = element.getAttribute(ATTRIBUTE_COMPONENT_REF);

            if (!config.componentValues().contains(ref)) {
                throw new RuntimeException("Invalid component reference: " + ref);
            }
    ...
    private static List<XmlCategory> parseCategories(Element componentElement, ReducedConfig config) {
        List<XmlCategory> categories = new ArrayList<>();

        NodeList categoryNodes = componentElement.getElementsByTagName(TAG_CATEGORY);
        ...
        for (int i = 0; i < categoryNodes.getLength(); i++) {
            ...
            String category = element.getAttribute(ATTRIBUTE_CATEGORY_NAME);

            if (!config.categoryValues().contains(category)) {
                throw new RuntimeException("Invalid category specified: " + category);
            }
    ...
    private static List<XmlBreaking> parseBreakings(Element categoryElement, ReducedConfig config) {
        ...
        for (int i = 0; i < breakingNodes.getLength(); i++) {
            Node node = breakingNodes.item(i);
            ...
            String severity = element.getAttribute(ATTRIBUTE_BREAKING_SEVERITY);
    ...
    private static EntityResolver createResolver() {
        return (_, systemId) -> {
            ...
            InputStream resource = PatchParserV2.class.getResourceAsStream(resourcePath);
